...
const Sidebar: React.FC = () => {
  ...
  return (
    <div className="flex flex-col gap-10">
      ...
      {/* Core Competencies */}
      <section className="page-break-avoid">
        <h3 className="text-[#1e3a8a] text-sm font-bold tracking-widest uppercase mb-6 border-b border-gray-200 pb-1">
          Core Competencies
        </h3>
        <div className="flex flex-col gap-6">
          {competencies.map((group, idx) => (
            <div key={idx}>
              <h4 className="font-bold text-xs text-gray-900 uppercase mb-2 tracking-wider">{group.title}</h4>
              <ul className="flex flex-wrap gap-2">
                {group.skills.map((skill, sIdx) => (
                  <li key={sIdx} className="bg-white border border-gray-200 px-2 py-1 text-[11px] font-medium text-gray-600 rounded">
                    {skill}
                  </li>
                ))}
              </ul>
